...
></div>
  <div class="wrapper">
      ...
      <div class="shop_header_info">
        <div class="shop_header_info_body">
          ...
          <div class="shop_header_info_body_situation">
            <div class="shop_intro">{{ shop.shopIntro }}</div>
          </div>
    ...
    <div class="shop">
      <div class="shop_choice">
        ...
        <div class="shop_choice_right">
          <span 
            class="shop_choice_right_together"
            @click="toMyOrder()"
          >我的订单</span>
        </div>
      </div>
      <div class="shop_order">
        ...
        <div class="shop_order_content">
          <div class="shop_order_content_title">{{ cateName }}</div>
          <div
          ...
          >
            ...
            <div class="shop_order_content_item_body">
              ...
              <div class="goods_count">
                {{ productList[goods.goodsId]?.count || 0 }}
              </div>
              <div
    ...
    <div class="cart" v-show="showMask">
      ...
      >
        ...
        <div class="cart_item_body">
          ...
          <div class="cart_item_body_goods_count">{{ product.count || 0 }}</div>
          <div
    ...
    <!-- shop_cart -->
    <div class="shop_cart">
      <div class="shop_cart_icon iconfont" @click="handleMask()">&#xe618;</div>
...
const useMaksEffect = () => {
    ...
    return { showMask, handleMask }
}

// getGoodsWithCateByShopId逻辑
const getGoodsWithCateByShopId = () => {
  ...
  const shop = ref({});
  // 获取shopName
  const shopName = shop.value.shopName || ''
  // cateList
  ...
  const getData = async(shopId) => {
      const result = await getRequest('/shop/getShopDetail.php', {
        shopId: shopId
      })
      // console.log(result)
      if (result?.code && result?.code === 200) {
        shop.value = result?.object
        cateList.values = result?.object?.cateList
        goodsList.values = result?.object?.cateList[0]?.goods
      }
  }
    ...
    const changeCate = (index) => {
        ...
        cateName.value = cateList.values[index].cateName

        goodsList.values = cateList.values[index].goods
    }

  return { shop, cateList, goodsList, activeIndex, cateName, changeCate,
   shopId, shopName };
};
...
const toEffect = () => {
    ...
    return { checkOut, handleBack, toMyOrder }
}
...
export default {
  ...
  setup() {
      // mask逻辑
      const { showMask, handleMask } = useMaksEffect()
      // getGoodsWithCateByShopId逻辑
      const { shop, cateList, goodsList, shopId, shopName,
      activeIndex, cateName, changeCate } = getGoodsWithCateByShopId()
      // 跳转逻辑
      const { checkOut, handleBack, toMyOrder } = toEffect()
      // 购物车逻辑
      const { changeCartItemInfo, total, clearCart, productList } = useCartEffect(shopId)
      

      return { shop, cateList, goodsList, showMask, handleMask, checkOut, handleBack,
      activeIndex, cateName, changeCate, changeCartItemInfo, shopId, shopName,
      productList, total, clearCart, toMyOrder }
  },
};
</script>
