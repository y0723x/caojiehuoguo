      ...
      >
        <div class="order_container_item_title">
          <div
            ...
            @click="toShop(order.shopId, order.tableId)"
          >
            <img
...
const useOrderListEffect = () => {
  ...
  const handleHeader = (index) => {
    ...
    if (status.value != -1) {
      ...
      orderList.values.forEach((order) => {
        if (order.status === String(status.value)) {
  ...
  const getAllOrder = async () => {
    const result = await getRequest("/order/getAllOrder.php");
